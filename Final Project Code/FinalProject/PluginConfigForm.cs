...
namespace FinalProject
{
    public partial class PluginConfigForm : Form
    {
        ...
        public PluginConfigForm(ConfigurablePluginIF pluginInstance, Dictionary<string, string> initialConfig = null)
        {
            InitializeComponent();
            this.pluginInstance = pluginInstance;
            labelPluginName.Text = "Config"+pluginInstance.GetName();
            LoadParameters(initialConfig);
        }
        ...
        private void LoadParameters(Dictionary<string, string> initialConfig)
        {
            ...
            var requiredParams = pluginInstance.GetRequiredParameters();
            ...
            var optionalParams = pluginInstance.GetOptionalParameters();
            foreach (var entry in optionalParams)
            {
                string initialValue = initialConfig != null && initialConfig.TryGetValue(entry.Key, out string val) ? val : entry.Value;
        ...
        private void buttonAddPlugin_Click(object sender, EventArgs e)
        {
            ...
            foreach (var key in pluginInstance.GetRequiredParameters().Keys)
            {
            ...
            pluginInstance.SetParameters(required, optional);
